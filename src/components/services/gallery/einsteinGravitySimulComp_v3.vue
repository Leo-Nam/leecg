<script setup>
// 일정한 중력값 이상의 질량을 가진 물체가 임계값 이상에서 폭발하는 시뮬레이션 추가
...
const mistGasNum = ref(100); // 원하는 미세먼지 개수 설정 (Vue에서는 ref로 바인딩 가능)
...
function draw() {
  ...
  for (let sphere of spheres.value) {
    ...
    const blueIntensity = Math.max(0, 255 - sphere.mass / 50);
    ctx.fillStyle = `rgba(${redIntensity}, 150, ${blueIntensity}, 0.8)`; // 물체 색상 변경
    ctx.fill();
